...
const InspectionItemList = ({ inspection }: IInspection) => {
    ...
    return (
        <li key={inspection.id} className={`w-full h-[50px] px-10 flex justify-between items-center ${isDark ? 'bg-backDark text-labelDark border-borderDark' : 'bg-back text-label border-border'} border-b-[1px] ${isDark ? 'hover:bg-hoverDark' : 'hover:bg-hover'} transition ease-in duration-500`}>
            ...
            <span className='w-[30%] font-normal'>ID: <span className='font-semibold'>{inspection.id}</span></span>
            <button onClick={() => {
                setModalIsOpen(!modalIsOpen)
                setHandleUrls(inspection.urls)
            }} className='flex justify-center items-center font-semibold bg-logo px-2 text-white rounded-sm'><FiPlusCircle className='mr-1' />urls</button>
            <div className='w-[180px] flex justify-between items-center'>
                <button onClick={() => retrieveInspectionById(inspection.id)} className={`flex justify-center items-center text-[14px] text-logo  ${inspection.status === 'done' && 'opacity-25 cursor-not-allowed'}`}>
