        ...
        <div className={`w-full max-w-[1000px] h-[180px] my-4 ${isDark ? 'bg-backFormDark' : 'bg-backForm'} shadow transition ease-in duration-500`}>
            ...
            <form onSubmit={handleSubmit(handleNewInspection)} className={`px-10 w-full flex items-end`}>
                <div className={`flex flex-col`}>
                    ...
                    <input type="text" {...register("keyword")} className={`h-[40px] rounded-sm mt-2 pl-4 outline-none font-normal ${isDark ? 'bg-backDark text-white' : 'bg-white text-title'} transition ease-in duration-500`} placeholder={'Ex: Security'}/>
                </div>
